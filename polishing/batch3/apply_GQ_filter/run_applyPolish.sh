...
cd apply_GQ_filter/HG02027/filtered_vcf
ls *.vcf.gz | while read line ; do echo $line ; zcat $line | grep -v "^#" | wc -l ; done

# check filters worked - empty vcf produced
bcftools view -i 'FORMAT/GQ<20 && (ILEN = 1)' HG02027.polisher_output.GQ_filtered.vcf.gz
bcftools view -i 'FORMAT/GQ<12 && (ILEN = -1)' HG02027.polisher_output.GQ_filtered.vcf.gz
bcftools view -i 'FORMAT/GQ<=5' HG02027.polisher_output.GQ_filtered.vcf.gz
...
cd /Users/miramastoras/Desktop/Paten_lab/hprc_intermediate_assembly/polishing/batch3/apply_GQ_filter/

# create copy of csv
cp HPRC_Intermediate_Assembly_s3Locs_Batch2.updated.noTopUp.updated.csv HPRC_Intermediate_Assembly_s3Locs_Batch2.updated.noTopUp.updated.filterVcf.csv
# open in excel and ad new column of NAs, DeepPolisherFilteredVcf
head -n 1 HPRC_Intermediate_Assembly_s3Locs_Batch2.updated.noTopUp.updated.filterVcf.csv > tmp.csv
grep -v "sample_id" HPRC_Intermediate_Assembly_s3Locs_Batch2.updated.noTopUp.updated.filterVcf.csv \
| cut -f1 -d"," | while read line ; do grep $line HPRC_Intermediate_Assembly_s3Locs_Batch2.updated.noTopUp.updated.filterVcf.csv \
| sed "s|NA|/private/groups/hprc/polishing/batch3/apply_GQ_filter/${line}/filtered_vcf/${line}.polisher_output.GQ_filtered.vcf.gz|g" >> tmp.csv; done

mv tmp.csv HPRC_Intermediate_Assembly_s3Locs_Batch2.updated.noTopUp.updated.filterVcf.csv
###############################################################################
##                             create input jsons                            ##
###############################################################################
...
mkdir -p  /Users/miramastoras/Desktop/Paten_lab/hprc_intermediate_assembly/polishing/batch3/apply_GQ_filter/applyPolish_input_jsons
cd /Users/miramastoras/Desktop/Paten_lab/hprc_intermediate_assembly/polishing/batch3/apply_GQ_filter/applyPolish_input_jsons

python3 /Users/miramastoras/Desktop/Paten_lab/hprc_intermediate_assembly/hpc/launch_from_table.py \
     --data_table ../HPRC_Intermediate_Assembly_s3Locs_Batch2.updated.noTopUp.updated.filterVcf.csv \
...
# move to work dir
cd /private/groups/hprc/polishing/batch3/apply_GQ_filter

## get files to run in polishing folder ...
cp -r /private/groups/hprc/polishing/hprc_intermediate_assembly/polishing/batch3/apply_GQ_filter/* ./
...
sbatch \
     ...
     HPRC_Intermediate_Assembly_s3Locs_Batch2.updated.noTopUp.updated.filterVcf.csv
#
###############################################################################
